...
var parseArray = (jsonSchema, refs) => {
  ...
  let zodSchema = !jsonSchema.items ? z3.array(z3.any()) : z3.array(parseSchema(jsonSchema.items, { ...refs, path: [...refs.path, "items"] }));
...
var parseConst = (jsonSchema) => {
  return z5.literal(jsonSchema.const);
};
...
var parseEnum = (jsonSchema) => {
  ...
  if (jsonSchema.enum.length === 1) {
    return z7.literal(jsonSchema.enum[0]);
  }
  if (jsonSchema.enum.every((x) => typeof x === "string")) {
    return z7.enum(jsonSchema.enum);
  }
  return z7.union(
    jsonSchema.enum.map((x) => z7.literal(x))
  );
};
...
var parseIfThenElse = (jsonSchema, refs) => {
  ...
  return z8.union([$then, $else]).superRefine((value, ctx) => {
...
var parseOneOf = (jsonSchema, refs) => {
  ...
  return z13.any().superRefine((x, ctx) => {
