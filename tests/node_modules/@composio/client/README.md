...
[![NPM version](<https://img.shields.io/npm/v/@composio/client.svg?label=npm%20(stable)>)](https://npmjs.org/package/@composio/client) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/@composio/client)
...
const response: Composio.ToolExecuteResponse = await client.tools.execute('tool_slug');
```
...
<!-- prettier-ignore -->
```ts
const response = await client.tools.execute('tool_slug').catch(async (err) => {
...
await client.tools.execute('tool_slug', {
  maxRetries: 5,
});
```
...
const response = await client.tools.execute('tool_slug').asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: response, response: raw } = await client.tools.execute('tool_slug').withResponse();
