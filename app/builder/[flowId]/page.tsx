...
export default function BuilderPage() {
  ...
  useEffect(() => {
    const fetchFlowName = async () => {
      // For now, use localStorage like the original
      // Later this will be replaced with database calls
      const flowData = localStorage.getItem(`flow_${flowId}`);
      if (flowData) {
        const flow = JSON.parse(flowData);
        setFlowName(flow.name || `Flow ${flowId}`);
        ...
        localStorage.setItem(`flow_${flowId}`, JSON.stringify(newFlow));
      }
  ...
  const handleNameSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    localStorage.setItem(`flow_${flowId}`, JSON.stringify(flow));
  };
